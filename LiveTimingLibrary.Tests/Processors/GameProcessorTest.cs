...
public class TestGameProcessor()
{
    ...
    public void TestHandleSessionIdChange()
    {
        ...
        gameData.NewData.TrackName = "Testtrack 2";
        processor.Run(gameData);
        mockPropertyManager.Verify(m => m.ResetAll(), Times.Exactly(2));
        mockRaceEventHandler.Verify(m => m.ReinitPitEventStore(), Times.Exactly(2));
        mockRaceEventHandler.Verify(m => m.ReinitPlayerFinishedLapEventStore(), Times.Exactly(2));

        // new GameName -> -> should handle session change again
        gameData.NewData.GameName = "Gamename 2";
        processor.Run(gameData);
        mockPropertyManager.Verify(m => m.ResetAll(), Times.Exactly(3));
        mockRaceEventHandler.Verify(m => m.ReinitPitEventStore(), Times.Exactly(3));
        mockRaceEventHandler.Verify(m => m.ReinitPlayerFinishedLapEventStore(), Times.Exactly(3));
    }
